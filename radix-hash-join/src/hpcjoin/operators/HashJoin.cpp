...
HashJoin::HashJoin(uint32_t numberOfNodes, uint32_t nodeId,
...
void HashJoin::join() {
  ...
  hpcjoin::tasks::HistogramComputation *histogramComputation =
      new hpcjoin::tasks::HistogramComputation(
  ...
  hpcjoin::data::Window *outerWindow = new hpcjoin::data::Window(
      this->numberOfNodes, this->nodeId, histogramComputation->getAssignment(),
  ...
  hpcjoin::tasks::NetworkPartitioning *networkPartitioning =
      new hpcjoin::tasks::NetworkPartitioning(this->nodeId, this->innerRelation,
                                              this->outerRelation, innerWindow,
                                              outerWindow);
  ...
  if (hpcjoin::core::Configuration::ENABLE_TWO_LEVEL_PARTITIONING) {
    // hpcjoin::memory::Pool::allocate((innerWindow->computeLocalWindowSize() +
    // outerWindow->computeLocalWindowSize())*sizeof(hpcjoin::data::Tuple));
  ...
  for (uint32_t p = 0;
    ...
    if (assignment[p] == this->nodeId) {
      hpcjoin::data::CompressedTuple *innerRelationPartition =
          innerWindow->getPartition(p);
      uint64_t innerRelationPartitionSize = innerWindow->getPartitionSize(p);
      hpcjoin::data::CompressedTuple *outerRelationPartition =
          outerWindow->getPartition(p);
      ...
      if (hpcjoin::core::Configuration::ENABLE_TWO_LEVEL_PARTITIONING) {
        ...
        TASK_QUEUE.push(new hpcjoin::tasks::BuildProbe(
            innerRelationPartitionSize, innerRelationPartition,
            outerRelationPartitionSize, outerRelationPartition));
      }
    }
  ...
  while (TASK_QUEUE.size() > 0) {
    ...
    // OPTIMIZATION When second partitioning pass is completed, windows are no
    // longer required
    if (hpcjoin::core::Configuration::ENABLE_TWO_LEVEL_PARTITIONING &&
        windowsDeleted) {
