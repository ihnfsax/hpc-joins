...
void OffsetMap::computeBaseOffsets() {
  ...
  uint64_t *currentOffsets =
      (uint64_t *)calloc(this->numberOfProcesses, sizeof(uint64_t));
  ...
  for (uint32_t i = 0;
       i < hpcjoin::core::Configuration::NETWORK_PARTITIONING_COUNT; ++i) {
...
void OffsetMap::computeRelativePrivateOffsets() {
  ...
  MPI_Scan(this->localHistogram->getLocalHistogram(),
...
void OffsetMap::computeAbsolutePrivateOffsets() {
  ...
  for (uint32_t i = 0;
       i < hpcjoin::core::Configuration::NETWORK_PARTITIONING_COUNT; ++i) {
    this->absoluteWriteOffsets[i] =
        this->baseOffsets[i] + this->relativeWriteOffsets[i];
  }
}

uint64_t *OffsetMap::getBaseOffsets() { return baseOffsets; }

uint64_t *OffsetMap::getRelativeWriteOffsets() { return relativeWriteOffsets; }

uint64_t *OffsetMap::getAbsoluteWriteOffsets() { return absoluteWriteOffsets; }
