...
LocalHistogram::LocalHistogram(hpcjoin::data::Relation *relation) {
  ...
  this->values = (uint64_t *)calloc(
      hpcjoin::core::Configuration::NETWORK_PARTITIONING_COUNT,
      sizeof(uint64_t));
}

LocalHistogram::~LocalHistogram() { free(values); }
...
void LocalHistogram::computeLocalHistogram() {
  ...
  for (uint64_t i = 0; i < numberOfElements; ++i) {
    uint32_t partitionIdx = HASH_BIT_MODULO(
        data[i].key,
...
#ifdef MEASUREMENT_DETAILS_HISTOGRAM
  hpcjoin::performance::Measurements::stopHistogramLocalHistogramComputation(
      numberOfElements);
#endif
}

uint64_t *LocalHistogram::getLocalHistogram() { return this->values; }
