...
void BuildProbe::execute() {
  ...
  JOIN_DEBUG("Build-Probe", "Executing build-probe phase of size %lu x %lu",
             innerPartitionSize, outerPartitionSize);

  uint32_t const keyShift =
      hpcjoin::core::Configuration::NETWORK_PARTITIONING_FANOUT +
      hpcjoin::core::Configuration::PAYLOAD_BITS;
  uint32_t const shiftBits =
      keyShift + hpcjoin::core::Configuration::LOCAL_PARTITIONING_FANOUT;
  ...
  uint64_t *hashTableNext =
      (uint64_t *)calloc(this->innerPartitionSize, sizeof(uint64_t));
  uint64_t *hashTableBucket = (uint64_t *)calloc(N, sizeof(uint64_t));

#ifdef MEASUREMENT_DETAILS_LOCALBP
  hpcjoin::performance::Measurements::stopBuildProbeMemoryAllocation(
...
#ifdef MEASUREMENT_DETAILS_LOCALBP
  hpcjoin::performance::Measurements::stopBuildProbeBuild(
      this->innerPartitionSize);
#endif
  ...
  for (uint64_t t = 0; t < this->outerPartitionSize; ++t) {
    ...
    for (uint64_t hit = hashTableBucket[idx]; hit > 0;
         hit = hashTableNext[hit - 1]) {
      if ((outerPartition[t].value >> keyShift) ==
          (innerPartition[hit - 1].value >> keyShift)) {
        ++matches;
      }
    }
  }
...
#ifdef MEASUREMENT_DETAILS_LOCALBP
  hpcjoin::performance::Measurements::stopBuildProbeProbe(
...
task_type_t BuildProbe::getType() { return TASK_BUILD_PROBE; }
