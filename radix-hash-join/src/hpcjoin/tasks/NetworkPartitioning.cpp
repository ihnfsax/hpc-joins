...
#define TUPLES_PER_CACHELINE                                                   \
  (NETWORK_PARTITIONING_CACHELINE_SIZE / sizeof(hpcjoin::data::CompressedTuple))

#define HASH_BIT_MODULO(KEY, MASK, NBITS) (((KEY) & (MASK)) >> (NBITS))

#define PARTITION_ACCESS(p)                                                    \
  (((char *)inMemoryBuffer) +                                                  \
   (p * hpcjoin::core::Configuration::MEMORY_PARTITION_SIZE_BYTES))
...
NetworkPartitioning::NetworkPartitioning(uint32_t nodeId,
  ...
  JOIN_ASSERT(hpcjoin::core::Configuration::CACHELINE_SIZE_BYTES ==
                  NETWORK_PARTITIONING_CACHELINE_SIZE,
...
void NetworkPartitioning::execute() {
  ...
  JOIN_DEBUG("Network Partitioning", "Node %d is partitioning inner relation",
             this->nodeId);
  partition(innerRelation, innerWindow);

  JOIN_DEBUG("Network Partitioning", "Node %d is partitioning outer relation",
             this->nodeId);
...
void NetworkPartitioning::partition(hpcjoin::data::Relation *relation,
  ...
  JOIN_ASSERT(result == 0, "Network Partitioning",
              "Could not allocate in-memory buffer");
  memset(inMemoryBuffer, 0, inMemoryBufferSize);

#ifdef MEASUREMENT_DETAILS_NETWORK
  hpcjoin::performance::Measurements::stopNetworkPartitioningMemoryAllocation(
      inMemoryBufferSize);
#endif
  ...
  for (uint64_t i = 0; i < numberOfElements; ++i) {
    ...
    if (inCacheCounter == TUPLES_PER_CACHELINE) {
      ...
      // JOIN_DEBUG("Network Partitioning", "Node %d has a full cache line %d",
      // this->nodeId, partitionId);
      ...
      // JOIN_DEBUG("Network Partitioning", "Node %d has completed the stream
      // write of cache line %d", this->nodeId, partitionId);
      ...
      if (memoryCounter %
        ...
        window->write(
            partitionId, inMemoryBufferLocation,
            hpcjoin::core::Configuration::CACHELINES_PER_MEMORY_BUFFER *
        ...
        // JOIN_DEBUG("Network Partitioning", "Node %d has completed the put
        // operation of memory buffer %d", this->nodeId, partitionId);
      }
  ...
  JOIN_DEBUG("Network Partitioning", "Node %d is flushing remaining tuples",
             this->nodeId);

#ifdef MEASUREMENT_DETAILS_NETWORK
  hpcjoin::performance::Measurements::
      startNetworkPartitioningFlushPartitioning();
#endif
  ...
  for (uint32_t p = 0;
    ...
    hpcjoin::data::CompressedTuple *cacheLine =
        (hpcjoin::data::CompressedTuple *)(inCacheBuffer + p);
    hpcjoin::data::CompressedTuple *inMemoryFreeSpace =
        reinterpret_cast<hpcjoin::data::CompressedTuple *>(
            PARTITION_ACCESS(p) +
            (memoryCounter * NETWORK_PARTITIONING_CACHELINE_SIZE));
    ...
    if (remainingTupleInMemory > 0) {
      hpcjoin::data::CompressedTuple *inMemoryBufferOfPartition =
          reinterpret_cast<hpcjoin::data::CompressedTuple *>(
              PARTITION_ACCESS(p) +
              (memoryCounter /
               hpcjoin::core::Configuration::CACHELINES_PER_MEMORY_BUFFER) *
                  hpcjoin::core::Configuration::MEMORY_BUFFER_SIZE_BYTES);
      window->write(p, inMemoryBufferOfPartition, remainingTupleInMemory,
                    false);
    }
  }
...
#ifdef MEASUREMENT_DETAILS_NETWORK
  hpcjoin::performance::Measurements::
      stopNetworkPartitioningFlushPartitioning();
#endif
...
inline void NetworkPartitioning::streamWrite(void *to, void *from) {
  ...
  JOIN_ASSERT(to != NULL, "Network Partitioning",
              "Stream destination should not be NULL");
  JOIN_ASSERT(from != NULL, "Network Partitioning",
              "Stream source should not be NULL");

  JOIN_ASSERT(((uint64_t)to) % NETWORK_PARTITIONING_CACHELINE_SIZE == 0,
              "Network Partitioning", "Stream destination not aligned");
  JOIN_ASSERT(((uint64_t)from) % NETWORK_PARTITIONING_CACHELINE_SIZE == 0,
              "Network Partitioning", "Stream source not aligned");
...
task_type_t NetworkPartitioning::getType() { return TASK_NET_PARTITION; }
