...
HistogramComputation::HistogramComputation(
    uint32_t numberOfNodes, uint32_t nodeId,
  ...
  this->innerRelationLocalHistogram =
      new hpcjoin::histograms::LocalHistogram(innerRelation);
  this->outerRelationLocalHistogram =
      new hpcjoin::histograms::LocalHistogram(outerRelation);
  ...
  this->assignment = new hpcjoin::histograms::AssignmentMap(
      this->numberOfNodes, this->innerRelationGlobalHistogram,
      this->outerRelationGlobalHistogram);

  this->innerOffsets = new hpcjoin::histograms::OffsetMap(
      this->numberOfNodes, this->innerRelationLocalHistogram,
      this->innerRelationGlobalHistogram, this->assignment);
  this->outerOffsets = new hpcjoin::histograms::OffsetMap(
      this->numberOfNodes, this->outerRelationLocalHistogram,
      this->outerRelationGlobalHistogram, this->assignment);
}
...
task_type_t HistogramComputation::getType() { return TASK_HISTOGRAM; }
