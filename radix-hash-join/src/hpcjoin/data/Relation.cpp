...
Relation::Relation(uint64_t localSize, uint64_t globalSize) {
  ...
  // int result = posix_memalign((void **) &(this->data),
  // hpcjoin::core::Configuration::CACHELINE_SIZE_BYTES, localSize *
  // sizeof(hpcjoin::data::Tuple)); JOIN_ASSERT(result == 0, "Relation", "Could
  // not allocate memory for %lu tuples", localSize);
...
Relation::~Relation() { free(this->data); }

uint64_t Relation::getLocalSize() { return this->localSize; }

uint64_t Relation::getGlobalSize() { return this->globalSize; }

hpcjoin::data::Tuple *Relation::getData() { return this->data; }

void Relation::fillUniqueValues(uint64_t startKeyValue,
...
void Relation::fillModuloValues(uint64_t startKeyValue, uint64_t startRidValue,
...
void Relation::distribute(uint32_t nodeId, uint32_t numberOfNodes) {
  ...
  for (uint32_t i = 0; i < nodeId; ++i) {
    ...
    uint64_t sectionSize =
        (section == numberOfNodes - 1)
            ? (localSize - (numberOfNodes - 1) * (localSize / numberOfNodes))
            : (localSize / numberOfNodes);
    JOIN_DEBUG("SWAP", "%d (%lu) <--> %d (%lu)\n", nodeId, section, i, section);
    MPI_Recv(incomingData, sectionSize * sizeof(hpcjoin::data::Tuple), MPI_BYTE,
             i, EXCHANGE_DATA_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Send(this->data + sectionStart,
             sectionSize * sizeof(hpcjoin::data::Tuple), MPI_BYTE, i,
