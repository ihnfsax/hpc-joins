...
Window::Window(uint32_t numberOfNodes, uint32_t nodeId, uint32_t *assignment,
               uint64_t *localHistogram, uint64_t *globalHistogram,
               uint64_t *baseOffsets, uint64_t *writeOffsets) {
  ...
  MPI_Alloc_mem(localWindowSize * sizeof(hpcjoin::data::CompressedTuple),
                MPI_INFO_NULL, &(this->data));
#ifdef USE_FOMPI
  foMPI_Win_create(this->data,
                   localWindowSize * sizeof(hpcjoin::data::CompressedTuple), 1,
  ...
  JOIN_DEBUG("Window", "Window is at address %p to %p", this->data,
             this->data + localWindowSize);
}
...
void Window::write(uint32_t partitionId, CompressedTuple *tuples,
                   uint64_t sizeInTuples, bool flush) {

  // JOIN_DEBUG("Window", "Initializing write for partition %d of %lu tuples",
  // partitionId, sizeInTuples);
  ...
  // JOIN_DEBUG("Window", "Target %d and offset %lu (%lu + %lu)", targetProcess,
  // targetOffset, this->writeOffsets[partitionId],
  // this->writeCounters[partitionId]);

#ifdef JOIN_DEBUG_PRINT
  uint64_t remoteSize = computeWindowSize(targetProcess);
#endif
  JOIN_ASSERT(targetOffset <= remoteSize, "Window",
              "Target offset is outside window range");
  JOIN_ASSERT(targetOffset + sizeInTuples <= remoteSize, "Window",
              "Target offset and size is outside window range");

#ifdef USE_FOMPI
  foMPI_Put(tuples, sizeInTuples * sizeof(CompressedTuple), MPI_BYTE,
            targetProcess, targetOffset * sizeof(CompressedTuple),
            sizeInTuples * sizeof(CompressedTuple), MPI_BYTE, *window);
#else
  MPI_Put(tuples, sizeInTuples * sizeof(CompressedTuple), MPI_BYTE,
          targetProcess, targetOffset * sizeof(CompressedTuple),
          sizeInTuples * sizeof(CompressedTuple), MPI_BYTE, *window);
#endif

  this->writeCounters[partitionId] += sizeInTuples;
  // JOIN_DEBUG("Window", "Partition %d has now %lu tuples", partitionId,
  // this->writeCounters[partitionId]);

  JOIN_ASSERT(this->writeCounters[partitionId] <=
                  this->localHistogram[partitionId],
              "Window",
              "Node %d is writing to partition %d. Has %lu tuples declared, "
              "but write counter is now %lu.",
...
void Window::assertAllTuplesWritten() {
  ...
  for (uint32_t p = 0;
       p < hpcjoin::core::Configuration::NETWORK_PARTITIONING_COUNT; ++p) {
    JOIN_ASSERT(this->localHistogram[p] == this->writeCounters[p], "Window",
                "Not all tuples submitted to window. Partition %d. Local size "
                "%lu tuples. Write size %lu tuples.",
